...
pub fn current_add_map_area(addr: Range<usize>, port: usize, exact: bool) -> Option<usize> {
    let task = current_task().unwrap();
    let ret = task.acquire_inner_lock().add_map_area(addr, port, exact); ret
}

pub fn current_remove_map_area(addr: Range<usize>) -> bool {
    let task = current_task().unwrap();
    let ret = task.acquire_inner_lock().remove_map_area(addr); ret
}
